...
public static class ProductsEndPoints
{
    ...
    )
    {
        ...
        return await productRepository.ExistsByIdAsync(productId)
            ? Results.Ok()
            : Results.NotFound();
    }
    ...
    )
    {
        ...
        if (includeReviews)
        {
            reviews = await productRepository.GetProductReviewsAsync(productId);
        }
    ...
    )
    {
        ...
        await productRepository.AddProductReviewAsync(productReview);
    ...
    )
    {
        ...
        int totalCount = await productRepository.ProductCountsAsync();

        var products = await productRepository.GetProductsPageAsync(page, pageSize);
    ...
    > CreateProduct(CreateProductRequest product, IProductRepository productRepository)
    {
        if (product is null)
            throw new ArgumentNullException(nameof(product), "Product cannot be null.");

        // Prevent duplicate products by name.
        if (await productRepository.ExistsByNameAsync(product.Name))
        {
    ...
    {
        var repoProduct = await productRepository.GetProductByIdAsync(productId);
        ...
        var succeeded = await productRepository.UpdateProductAsync(repoProduct);
    ...
    )
    {
        ...
        var success = await productRepository.UpdateProductAsync(product);
    ...
    )
    {
        ...
        var success = await productRepository.DeleteProductAsync(productId);

        return success ? Results.NoContent() : Results.StatusCode(500);
    }

    private static async Task<IResult> GetCsvFile(IProductRepository productRepository)
    {
        var products = await productRepository.GetProductsPageAsync(1, 100);
