...
internal class Program
{
    private static void Main(string[] args)
    {
        using (var context = new AppDbContext())
        {
            ...
            Console.WriteLine($"Single __ The section has id {section.Id} - name : {section.SectionName}");

            section = context.Sections.First(section => section.Id == 1);
            Console.WriteLine($"First __ The section has id {section.Id} - name : {section.SectionName}");
             ...
             */
            var SelectQuery = context.Sections
             ...
             * WARNING: This can cause massive performance hits if it downloads thousands of rows 
             * just to calculate something in memory.
             */
            var newSelectQuery = context.Sections
               .Where(section => section.Id == 1)
               .Select(static section => new
               {
                   Name = section.SectionName,
                   Hours = CalculateThePeriod(section.TimeSlot.StartTime, section.TimeSlot.EndTime)
               });
            ...
            Console.WriteLine($"After changes (NoTracking) : sectionName : {section.SectionName}\n");
             ...
             */
            var QuerySection = context.Sections
                .Where(section => section.Id == 6)
            ...
            Console.WriteLine($"The Students of this section {QuerySection.SectionName} (Eager Loading): ");
            foreach (var item in QuerySection.Enrollments)
                Console.WriteLine($"Enrollment ID : {item.SectionId},{item.StudentId} - StudentNAME : {item.Student.FirstName} {item.Student.LastName}");
            Console.WriteLine();
            ...
            var queryEnrollments = context.Entry(targetSection)
                                          .Collection(section => section.Enrollments)
                                          .Query()
                                          .Include(enrollment => enrollment.Student)
                                          .ToList();

            Console.WriteLine($"The Students of this section {targetSection.SectionName} (Explicit Loading): ");
             ...
             */
            var projectedSection = context.Sections
                .Where(section => section.Id == 6)
                .Select(section => new
                {
                    ...
                    StudentsList = section.Enrollments.Select(enrollment => new
                    {
            ...
            Console.WriteLine($"The Students of this section {projectedSection.SectionName} (Select Projection): ");
            foreach (var student in projectedSection.StudentsList)
            {
                Console.WriteLine($"Student ID : {student.StudentId} - Full Name : {student.FullName}");
            }
            Console.WriteLine();
            Console.ReadKey();
        }
